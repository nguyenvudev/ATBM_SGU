...
#         os.makedirs(private_key_filename, exist_ok=True)
#         with open(private_key_filename, 'w') as f:
#             f.write(pem_private)
#
#         # Gửi file khóa riêng cho người dùng
...
def decrypt_email(email_id):
    ...
    decrypted_body = None
    decrypted_attachment_base64 = None
            ...
            try:
                ...
                if action == 'decrypt_both':
                    ...
                    if email.attachment:
                        ...
                        decrypted_attachment_base64 = base64.b64encode(decrypted_attachment).decode('utf-8')
    ...
    return render_template('decrypt_email.html', email=email, decrypted_body=decrypted_body,
                           decryption_error=decryption_error, decrypted_attachment=decrypted_attachment_base64)
...
#                 decrypted_aes_key_bytes = bytes.fromhex(decrypted_aes_key)
#
#                 if action == 'decrypt_body':
#                     decrypted_body_bytes = aes_decrypt(bytes.fromhex(email.body), decrypted_aes_key_bytes)
#                     decrypted_body = decrypted_body_bytes.decode('utf-8')
#
#                 elif action == 'decrypt_attachment' and email.attachment:
#                     attachment_path = os.path.join('attachments', email.attachment)
#
#                     # Đọc tệp đã mã hóa
#                     with open(attachment_path, 'rb') as file:
    ...
    if request.method == 'POST':
        ...
        if email.attachment and private_key:
            ...
            try:
                # Giải mã khóa AES
                decrypted_aes_key = rsa_decrypt(email.aes_key, private_key)
                decrypted_aes_key_bytes = base64.b64decode(decrypted_aes_key)  # Đảm bảo rằng AES key được giải mã chính xác

                # Giải mã dữ liệu
                decrypted_body = aes_decrypt(encrypted_data, decrypted_aes_key_bytes)

                # Lưu file giải mã tạm thời
                decrypted_file_path = 'decrypted_' + email.attachment
